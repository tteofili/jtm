 ...
 */
package com.github.tteofili.jtm.pipeline.utils;
...
/**
 * Utilities for UIMA specific tasks
 */
public class UimaUtil {

  public static void setIssueId(CAS cas, Issue issue) {
    Type type = cas.getTypeSystem().getType("tm.Key");
    FeatureStructure fs = cas.createFS(type);
    fs.setStringValue(type.getFeatureByBaseName("id"), issue.getKey().getValue());
  ...
  public static String getIssueId(CAS cas) {
    Type type = cas.getTypeSystem().getType("tm.Key");
    FSIterator<FeatureStructure> fs = cas.getIndexRepository().getAllIndexedFS(type);
    FeatureStructure featureStructure = fs.get();
    return featureStructure.getStringValue(type.getFeatureByBaseName("id"));
  }
}
